...
void ManusDataPublisher::PublishCallback()
{
    ...
    {
        // 自动加载校准文件（按优先级搜索 Quantum > Pro > generic）
...
bool ManusDataPublisher::LoadCalibrationFile(uint32_t p_GloveId, Side p_Side)
{
    ...
    // 按优先级搜索校准文件 (Quantum > Pro > generic)
    std::string t_Side = (p_Side == Side_Left) ? "Left" : "Right";
    std::vector<std::string> t_Candidates = {
        t_PackageShareDir + "/calibration/" + t_Side + "QuantumMetaglove.mcal",
        t_PackageShareDir + "/calibration/" + t_Side + "MetaglovePro.mcal",
        t_PackageShareDir + "/calibration/Calibration.mcal",
    };

    std::string t_CalibrationPath;
    std::ifstream t_File;
    for (const auto& path : t_Candidates) {
        t_File.open(path, std::ios::binary);
        if (t_File) {
            t_CalibrationPath = path;
            break;
        }
    }

    if (!t_File) {
        ClientLog::warn("No calibration file found for {} glove. Run: ros2 run manus_ros2 manus_calibration", t_Side);
        return false;
    }

    ClientLog::print("Loading calibration from: {}", t_CalibrationPath);
